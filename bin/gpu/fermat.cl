 ...
 *      Author: mad
 */
#define BLOCK_SIZE 512  // Define the block size based on your GPU capabilities
__kernel 
                ...
                __global float4 *inputA)
{
    ...
    // Load input data into shared memory for better cache utilization
    __local float4 localInput[BLOCK_SIZE];
    localInput[get_local_id(0)] = inputA[gid];

    // Synchronize to ensure all data is loaded before processing
     barrier(0); 

    // Use vector indices for more efficient access
    uint4 result;
    float4 data = localInput[get_local_id(0)];
    
    // Perform element-wise addition with explicit type casting
    result.x = (uint)(data.x + data.x);
    result.y = (uint)(data.y + data.y);
    result.z = (uint)(data.z + data.z);
    result.w = (uint)(data.w + data.w);
...
uint int_invert(uint a, uint nPrime) {
    // Using the Extended Euclidean Algorithm to calculate the inverse of a in finite field defined by nPrime
...
__kernel void fermat_kernel(__global uchar* result, __global const uint4* fprimes) {
    ...
    result[id] = fermat352(p) ? 1 : 0;
}

__attribute__((reqd_work_group_size(256, 1, 1)))
__kernel void fermat_kernel320(__global uchar* result, __global const uint4* fprimes) {
    uint id = get_global_id(0);
    uint4 p[3];
    p[0] = fprimes[id * 3 + 0];
    p[1] = fprimes[id * 3 + 1];
